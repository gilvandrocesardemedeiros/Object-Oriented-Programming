...
bool Sudoku::resolver(bool com_exibicao)
{
  ...
  //Variavel booleana que verificarah quando o sudoku for resolvido
  bool resolvido = false;
  //Adicionando o sudoku inicial para a stack
  tentativas.push(*this);
  // Contadores do numero de tabuleiros gerados e testados
  int num_tab_gerados(1), num_tab_testados(0);
  //Elegendo melhor Sudoku como sendo o inicial (unico sudoku ateh agora)
  Sudoku Melhor(*this);
  //Declarando variavel que conta as casas vazias do melhor Sudoku
  int melhor_num_vazias = Melhor.num_casas_vazias();
  //Declarando variavel que conta quantas casas vazias o Sudoku atual possui
  int num_vazias;
  //Declarando Sudoku auxiliar
  Sudoku tab;
  //Indices do Sudoku
  int i,j;
  ...
  do
  {
    //Armazenando o Sudoku mais recente na variavel auxiliar
    tab = tentativas.top();
    //Resolvando casas faceis no Sudoku mais recente
    tab.resolver_casas_faceis();
    //Eliminando esse Sudoku do stack de Sudokus possiveis
    tentativas.pop();
    num_tab_testados++;
    //Verificacao se o Sudoku recente jah foi resolvido
    if(tab.fim_de_jogo()){
        resolvido = true;
        Melhor = tab;
        Melhor.exibir();
    ...
    Jogada J(i,j);
    //Verificacao e criacao de todos os Sudoku possiveis para essa casa
    for(int val=1; val<=9; val++)
    {
        ...
        {
            Sudoku tab_aux(tab);
            tab_aux.fazer_jogada(J);
            if(tab_aux.resolver_casas_faceis()<0)
            {
                if((num_vazias = tab_aux.num_casas_vazias()) < melhor_num_vazias)
                {
                    melhor_num_vazias = num_vazias;
                    Melhor = tab_aux;
                }
            }
            else
            {
                tentativas.push(tab_aux);
                num_tab_gerados++;
            }
        }
    }
}
    if((num_vazias = tab.num_casas_vazias()) < melhor_num_vazias){
        melhor_num_vazias = num_vazias;
        Melhor = tab;
        Melhor.exibir();
    }
...
}while(!tentativas.empty() && !resolvido);
  if(com_exibicao){
      Melhor.exibir();
  }
  return resolvido;
